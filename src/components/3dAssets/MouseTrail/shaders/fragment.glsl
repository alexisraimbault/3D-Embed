...
void main() {
    ...
    copyPosition += curl(copyPosition * curlSize, uTime, 0.1 + (1.0 - life) * 0.1) * speed;
    copyPosition += vec3(
        cos(aRandomness) * (aRandomness2 * (uTime - aDeltaLife)),
        sin(aRandomness) * (aRandomness2 * (uTime - aDeltaLife)),
        0
    );
    gl_Position = projectionMatrix * modelViewMatrix * vec4(copyPosition, 1.0);
    gl_PointSize = 8.0;
}   