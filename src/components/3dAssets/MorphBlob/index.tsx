...
const Blob = ({
    ...
    const color1 = hexToRgb(shapeSettings?.colors[0] || 'ffffff')
    const color2 = hexToRgb(shapeSettings?.colors[1] || 'ffffff')
    ...
    void main() {
        float dynamicIntensity = u_intensity * (0.5 + 0.5 * sin(u_time));
        float distort = 2.0 * vDisplacement * u_intensity * sin(vUv.y * 10.0 + u_time);
        
        // vec3 baseColor = vec3(0.2, 0.6, 1.0); // Base color

        vec3 color1 = vec3(${color1?.r}.0 / 255.0, ${color1?.g}.0 / 255.0, ${color1?.b}.0 / 255.0);
        vec3 color2 = vec3(${color2?.r}.0 / 255.0, ${color2?.g}.0 / 255.0, ${color2?.b}.0 / 255.0); 
        
        vec3 color = mix(color1, color2, abs(vUv.x - 0.5) * 2.0);
